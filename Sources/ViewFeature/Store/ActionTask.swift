...
extension ActionTask {
    ...
    public static func run(
        ...
        return ActionTask(storeTask: .run(id: taskId, operation: operation, onError: nil, cancelInFlight: false, priority: nil))
    }
    ...
    public func `catch`(_ handler: @escaping @MainActor (Error, State) -> Void) -> ActionTask {
        switch storeTask {
        case .run(let id, let operation, _, let cancelInFlight, let priority):
            return ActionTask(storeTask: .run(id: id, operation: operation, onError: handler, cancelInFlight: cancelInFlight, priority: priority))
        default:
            return self
        }
    }
    ...
    public func cancellable<ID: TaskID>(
        ...
        switch storeTask {
        case .run(_, let operation, let onError, _, let priority):
            let stringId = id.taskIdString
    ...
    ///
    /// Task priority determines the scheduling order of tasks. Use higher priorities
    ...
    public func priority(_ priority: TaskPriority) -> ActionTask {
        switch storeTask {
        case .run(let id, let operation, let onError, let cancelInFlight, _):
            return ActionTask(storeTask: .run(
                id: id,
                operation: operation,
                onError: onError,
